...
**Templates:** `kk:research`, `kk:cook`, `kk:code-review`, `kk:debug`
...
4. Do NOT fall back to subagents. `/kk:team` MUST use Agent Teams or abort.
...
## ON `/kk:team research <topic>` [--researchers N]:

*Wraps /kk:research skill — scope, gather, analyze, report.*
...
## ON `/kk:team cook <plan-path-or-description>` [--devs N]:

*Wraps /kk:cook skill — plan, code, test, review, finalize.*
...
5. **MONITOR** dev completion via TaskCompleted events:
   ...
   - Fallbakk: Check TaskList if no events received in 60s
   - `Task(subagent_type: "tester", model: "haiku", name: "tester")`
   - Tester runs full test suite, reports pass/fail

6. **DOCS SYNC EVAL** (MANDATORY for cook — from /kk:cook finalize):
   ```
...
## ON `/kk:team review <scope>` [--reviewers N]:

*Wraps /kk:code-review skill — scout, review, synthesize with evidence gates.*
...
## ON `/kk:team debug <issue>` [--debuggers N]:

*Wraps /kk:fix skill — root-cause-first, adversarial hypotheses, disprove to converge.*
...
5. **MONITOR** via TaskCompleted events. Debuggers should message each other — let them converge.
   ...
   - Fallbakk: Check TaskList if no events in 60s
