...
1. **Screenshots provided** → Use `kk:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `kk:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/kk:scout ext` (preferred) or `/kk:scout` to find files needing enhancement
