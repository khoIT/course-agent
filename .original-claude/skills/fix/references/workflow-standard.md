...
Activate `kk:debug` skill. Use `debugger` subagent if needed.
...
|------|------------------|
| 1 | `kk:debug`, `debugger` subagent |
| 2 | Multiple `Explore` subagents in parallel (optional) |
| 3 | `kk:problem-solving`, `kk:sequential-thinking`, parallel `Bash` for verification |
| 4 | `tester` subagent |
