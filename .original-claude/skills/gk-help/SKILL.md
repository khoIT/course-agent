---
name: kk:gk-help
description: "KhoiClaudeKit usage guide — discover commands, skills, workflows naturally."
argument-hint: "[category|command|task description]"
---

# KhoiClaudeKit Help

All-in-one KhoiClaudeKit guide. Run the script and present output based on type markers.
...
```bash
python .claude/skills/gk-help/scripts/gk-help.py "$ARGUMENTS"
```
...
- **`/kk:plan` → `/kk:cook`**: Best for high-risk or complex changes
- **`/kk:cook`**: Standalone for straightforward implementation
- **NEVER** claim `/kk:plan` is mandatory before `/kk:cook`
