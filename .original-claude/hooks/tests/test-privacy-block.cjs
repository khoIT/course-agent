...
const CK_CONFIG_PATH = path.join(__dirname, '..', '.gk.json');
...
// Test cases - config toggle (privacyBlogk: false)
const configToggleTests = [
  {
    name: 'privacyBlogk: false - .env should allow',
    input: { tool_input: { file_path: '.env' } },
    config: { privacyBlogk: false },
    expectBlogk: false
  },
  {
    name: 'privacyBlogk: false - credentials.json should allow',
    input: { tool_input: { file_path: 'credentials.json' } },
    config: { privacyBlogk: false },
    expectBlogk: false
  },
  {
    name: 'privacyBlogk: true - .env should block',
    input: { tool_input: { file_path: '.env' } },
    config: { privacyBlogk: true },
    expectBlogk: true
  }
];
...
async function main() {
  ...
  // Config toggle tests - requires temp directory with .claude/.gk.json
  ...
  for (const test of configToggleTests) {
    // Write test config
    fs.writeFileSync(
      path.join(tmpClaudeDir, '.gk.json'),
