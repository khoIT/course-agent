...
/**
 * Test script for .gkignore functionality
 * Tests that scout-block.sh respects .gkignore patterns
 */
...
const gkignorePath = path.join(__dirname, '..', '..', '.gkignore');
const gkignoreBackupPath = gkignorePath + '.backup';

// Backup original .gkignore if exists
let originalCkignore = null;
if (fs.existsSync(gkignorePath)) {
  originalCkignore = fs.readFileSync(gkignorePath, 'utf-8');
  fs.copyFileSync(gkignorePath, gkignoreBackupPath);
}
...
function writeCkignore(patterns) {
  fs.writeFileSync(gkignorePath, patterns.join('\n') + '\n');
}

function restoreCkignore() {
  if (originalCkignore !== null) {
    fs.writeFileSync(gkignorePath, originalCkignore);
    if (fs.existsSync(gkignoreBackupPath)) {
      fs.unlinkSync(gkignoreBackupPath);
    }
  }
}

console.log('Testing .gkignore functionality...\n');

let passed = 0;
let failed = 0;

// Test 1: Default patterns work (with existing .gkignore)
console.log('--- Test 1: Default patterns from .gkignore ---');
let result = runTest(
...
console.log('\n--- Test 2: Custom .gkignore with only "vendor" ---');
...
result = runTest(
  'node_modules ALLOWED when not in .gkignore',
  { tool_name: 'Read', tool_input: { file_path: 'node_modules/pkg.json' } },
  'ALLOWED'
);
if (result.success) {
