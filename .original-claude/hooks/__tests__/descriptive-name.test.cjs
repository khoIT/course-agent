...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .gk.json', async () => {
      // Create .claude/.gk.json with hook disabled
      const claudeDir = path.join(tempDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
        path.join(claudeDir, '.gk.json'),
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'gk-config-utils.cjs'),
        path.join(libDir, 'gk-config-utils.cjs')
      );
