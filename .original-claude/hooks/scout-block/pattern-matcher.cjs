 ...
 *
 * Uses 'ignore' package for .gkignore parsing and path matching.
...
// Default patterns if .gkignore doesn't exist or is empty
...
/**
 * Load patterns from .gkignore file
 * Falls back to DEFAULT_PATTERNS if file doesn't exist or is empty
 *
 * @param {string} gkignorePath - Path to .gkignore file
 * @returns {string[]} Array of patterns
 */
function loadPatterns(gkignorePath) {
  ...
  try {
    ...
    console.error('WARN: Failed to read .gkignore:', error.message);
    return DEFAULT_PATTERNS;
  }
}
 ...
 *
 * @param {string[]} patterns - Array of patterns from .gkignore
 ...
 *
 * @param {string[]} originalPatterns - Original patterns from .gkignore
