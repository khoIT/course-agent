/**
 * Shared utilities for KhoiClaudeKit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.gk.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.gk.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `gk-session-${sessionId}.json`);
}
 ...
 * @param {Object} config - KhoiClaudeKit config
 ...
 *   2. Global config (~/.claude/.gk.json) - user preferences
 *   3. Local config (./.claude/.gk.json) - project-specific overrides
 *
