...
/**
 * Load .gk.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .gk.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.gk.json')
      : path.join(process.cwd(), '.claude', '.gk.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .gk.json config
