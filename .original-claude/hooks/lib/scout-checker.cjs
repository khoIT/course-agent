 ...
 * @param {string} [params.options.gkignorePath] - Path to .gkignore file
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Resolve .gkignore path
  const resolvedCkignorePath = gkignorePath || path.join(claudeDir, '.gkignore');
