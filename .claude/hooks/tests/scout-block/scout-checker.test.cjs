 ...
 * Uses real .gkignore fixtures to validate end-to-end behavior.
 *
...
const DEFAULT_OPTS = {
  ...
  gkignorePath: path.join(FIXTURES_DIR, 'gkignore-default.txt'),
...
describe('checkScoutBlock - fail-open behavior', () => {
  ...
  it('uses defaults when gkignorePath missing', () => {
    const r = checkScoutBlock({ toolName: 'Bash', toolInput: { command: 'ls node_modules' }, options: { gkignorePath: '/nonexistent/.gkignore' } });
    assert.ok(r.blocked, 'Should still block using DEFAULT_PATTERNS');
  });
});

describe('checkScoutBlock - custom .gkignore', () => {
  const customOpts = {
    ...DEFAULT_OPTS,
    gkignorePath: path.join(FIXTURES_DIR, 'gkignore-custom.txt')
  };
...
describe('checkScoutBlock - negation .gkignore', () => {
  const negOpts = {
    ...DEFAULT_OPTS,
    gkignorePath: path.join(FIXTURES_DIR, 'gkignore-negation.txt')
  };
