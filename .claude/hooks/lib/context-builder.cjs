...
const USAGE_CACHE_FILE = path.join(os.tmpdir(), 'gk-usage-limits-cache.json');
...
function buildContextSection(sessionId) {
  ...
  try {
    const contextPath = path.join(os.tmpdir(), `gk-context-${sessionId}.json`);
