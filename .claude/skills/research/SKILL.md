...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.gk.json` (or `~/.claude/.gk.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.claude/.gk.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `kk:docs-seeker` skill to find read it.
