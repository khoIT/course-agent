...
```bash
# Option 1: Install via KhoiClaudeKit CLI (recommended)
...
Use `/kk:kanban` for quick access:
...
| Route | Description |
|-------|-------------|
| `/` or `/kk:kanban` | Dashboard view |
| `/kk:kanban?dir=<path>` | Dashboard for specific directory |
