...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `kk:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `kk:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `kk:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `kk:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
...
- Visual chekk: [pass | issues found]
