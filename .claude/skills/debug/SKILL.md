...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `kk:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `kk:docs-seeker` skill for package/plugin docs; `repomix` for codebase summary
- **Scouting:** `/kk:scout` or `/kk:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `kk:chrome-devtools` skill for visual verification (screenshots, console, network)
