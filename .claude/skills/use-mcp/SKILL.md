...
1. **Execute task via Gemini CLI** (using stdin pipe for MCP support):
   ...
   # Read model from .claude/.gk.json: gemini.model (default: gemini-3-flash-preview)
...
2. **Fallback to mcp-manager subagent** (if Gemini CLI unavailable):
   ...
   - If the subagent got issues with the scripts of `kk:mcp-management` skill, use `kk:mcp-builder` skill to fix them
