...
Usage:
    python gk-help.py                    # Overview with quick start
    python gk-help.py fix                # Category guide with workflow
    python gk-help.py plan validate      # Subcommand details
    python gk-help.py debug login error  # Task recommendations
    python gk-help.py auth               # Search (unknown word)
"""
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ck.json", ".gk.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .gk.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "ClaudeKit Configuration (.gk.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.claude/.gk.json`"),
            ("Local", "Override per-project in `./.claude/.gk.json`"),
...
def discover_skills(skills_dir: Path) -> dict:
    ...
    for skill_md in sorted(skills_dir.rglob("SKILL.md")):
        ...
        # Skip template and gk-help itself
        if skill_name in ('template-skill', 'gk-help'):
            continue
...
def show_config_guide() -> None:
    """Display comprehensive .gk.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# ClaudeKit Configuration (.gk.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.claude/.gk.json` (user preferences)")
    print("- Local: `./.claude/.gk.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.claude/.gk.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.claude/.gk.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.claude/.gk.json - applies everywhere")
    ...
    print("# Local (.claude/.gk.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.claude/.gk.json` - personal preference")
    print("- Local: `./.claude/.gk.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.gk.json`")
...
def main():
    ...
    # .claude/skills/gk-help/scripts/gk-help.py -> .claude/skills
    ...
    if input_str.lower() in ["config", "configuration", ".gk.json", "ck.json"]:
        show_config_guide()
        return
