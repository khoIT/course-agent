...
4. **Context reminder:** `/kk:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/kk:scout` to search codebase
...
7. **Context reminder:** `/kk:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/kk:cook --parallel {absolute-plan-path}/plan.md`
...
```
Skill(skill: "kk:plan:red-team", args: "{plan-directory-path}")
```
...
| `auto` | Use the `Skill` tool: `Skill(skill: "kk:plan:validate", args: "{plan-directory-path}")` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool to invoke `plan:validate`:
```
Skill(skill: "kk:plan:validate", args: "{plan-directory-path}")
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/kk:cook --auto {path}/plan.md` |
| hard | `/kk:cook {path}/plan.md` |
| parallel | `/kk:cook --parallel {path}/plan.md` |
| two | `/kk:cook {path}/plan.md` |
