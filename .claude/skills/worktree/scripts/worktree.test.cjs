...
const MONOREPO_DIR = '/home/kai/khoiclaudekit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'khoiclaudekit-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from khoiclaudekit-engineer submodule
  const submodulePath = '/home/kai/khoiclaudekit/khoiclaudekit-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'khoiclaudekit-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'khoiclaudekit-cli', 'Should find khoiclaudekit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'khoiclaudekit' matches multiple projects
  const result = run('create khoiclaudekit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
