...
const { loadConfig } = require('./hooks/lib/gk-config-utils.cjs');
...
async function main() {
  try {
    ...
    try {
      ...
      if (sessionId) {
        const sessionPath = path.join(os.tmpdir(), `gk-session-${sessionId}.json`);
    ...
    if (sessionId && contextSize > 0) {
      try {
        const contextDataPath = path.join(os.tmpdir(), `gk-context-${sessionId}.json`);
    ...
    try {
      const usageCachePath = path.join(os.tmpdir(), 'gk-usage-limits-cache.json');
    ...
    // Fallbagk: output minimal single line on any error
